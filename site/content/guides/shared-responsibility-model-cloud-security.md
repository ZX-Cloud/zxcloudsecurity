...
The shared responsibility model defines the division of security obligations between a cloud provider and its customers. The provider secures the underlying infrastructure — physical data centres, network fabric, and hypervisor — whilst the customer is responsible for everything built and configured on top of it. All three major providers use equivalent framing: AWS describes it as security of the cloud (provider) versus security in the cloud (customer); Azure and GCP draw the same distinction.

The exact boundary shifts with the service model. IaaS customers using EC2, Azure VMs, or GCP Compute Engine own the guest OS, runtime, application code, data, and all network configuration above the hypervisor. PaaS customers retain responsibility for application code, data, and service configuration. SaaS customers own data governance, identity configuration, and compliance evidence — the provider manages virtually everything else. A critical implication that is frequently misunderstood: cloud provider compliance certifications — ISO 27001, SOC 2 Type II, PCI DSS — cover the provider's infrastructure layer only. Customer workloads require independent compliance evidence for every control within the customer's responsibility boundary.
