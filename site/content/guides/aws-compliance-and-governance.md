...
date: 2026-06-20
description: "SCPs, Control Tower, AWS Config, and NCSC alignment — the AWS compliance and governance architecture that holds up under FCA, NHS, and ISO 27001 audit."
...
word_count: 2288
faqs:
  - question: "What is AWS Control Tower and when should I use it?"
    answer: "AWS Control Tower is a managed service that automates the setup and ongoing governance of a multi-account AWS environment, implementing AWS best practices for account structure, logging, networking, and security controls. It creates a landing zone with a management account, Log Archive account, and Audit account, deploys organisation-wide CloudTrail and AWS Config, and enforces governance controls via guardrails (preventive SCPs and detective Config rules). Use Control Tower when building a new AWS organisation or when standardising an existing multi-account estate — it is the fastest path to a governed baseline. Existing accounts can be enrolled into a Control Tower landing zone."
  - question: "What is the difference between a Service Control Policy (SCP) and an IAM policy?"
    answer: "A Service Control Policy (SCP) is an AWS Organizations policy that sets the maximum permission ceiling for all IAM principals in an account or Organizational Unit — it restricts what IAM policies can grant, without granting any permissions itself. An IAM policy grants specific permissions to a principal (user, role, or group). An IAM policy can only grant what the SCP allows: if an SCP denies s3:DeleteBucket, no IAM policy in that account can override that denial, even AdministratorAccess. SCPs are the primary preventive control mechanism for multi-account environments, used to enforce organisation-wide guardrails that individual account administrators cannot remove."
  - question: "How does AWS Config support compliance and governance?"
    answer: "AWS Config continuously records the configuration state of AWS resources and evaluates them against Config Rules — either AWS-managed rules (predefined checks for common security misconfigurations) or custom rules written with Lambda or Guard policy language. Non-compliant resources generate findings that flow into AWS Security Hub. Config also provides a configuration history timeline for each resource, enabling you to see what a resource looked like at any point in time — essential for compliance audits and incident investigations where you need to prove configuration state at a specific date."
  - question: "What compliance frameworks does AWS support for UK regulated organisations?"
    answer: "AWS holds certifications and attestations directly relevant to UK regulated organisations: ISO 27001 and ISO 27017 (cloud-specific), ISO 27018 (PII in cloud), SOC 2 Type II, PCI DSS Level 1 Service Provider, Cyber Essentials Plus, NHS Data Security and Protection Toolkit (DSPT), and G-Cloud 14 listing. AWS also produces NCSC Cloud Security Principles mapping documentation covering all 14 principles. For FCA-regulated firms, AWS provides FCA PS21/3 Operational Resilience guidance and supports DORA requirements. Note that AWS's certifications cover the provider layer; your organisation must still implement and certify controls for the workloads you run."
draft: false
---
