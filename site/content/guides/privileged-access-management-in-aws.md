...
word_count: 2586
faqs:
  - question: "What is privileged access management (PAM) in AWS?"
    answer: "Privileged access management (PAM) in AWS is the set of controls that governs how and when elevated permissions are granted, used, and revoked for cloud resources. It encompasses: eliminating standing privilege in favour of just-in-time (JIT) access; using IAM roles with time-limited sessions rather than long-term credentials; routing privileged access through auditable channels such as AWS Systems Manager Session Manager rather than SSH/RDP; enforcing MFA and approval workflows before privilege is elevated; and logging all privileged activity to an immutable audit trail. PAM reduces the risk of lateral movement and data exfiltration by limiting how long and how broadly elevated permissions exist."
  - question: "What is just-in-time (JIT) access in AWS and how do I implement it?"
    answer: "Just-in-time access means privileged permissions are granted only for the duration of a specific task and automatically revoked afterwards — no principal holds elevated standing privilege between tasks. In AWS, JIT access is implemented by: using IAM Identity Centre session duration limits (1–4 hours) so temporary credentials expire automatically; integrating with a PAM tool (AWS Systems Manager Change Manager, CyberArk, BeyondTrust) that requires approval before a user can assume a high-privilege role; and using IAM role chaining with short session durations for break-glass scenarios. JIT eliminates the risk of long-term credential theft because the privileged credentials only exist for the duration of the approved session."
  - question: "How does AWS Systems Manager Session Manager replace bastion hosts?"
    answer: "AWS Systems Manager Session Manager provides secure, audited shell access to EC2 instances without opening inbound SSH or RDP ports, without a bastion host, and without managing SSH keys. Session Manager uses the SSM Agent (pre-installed on Amazon Linux 2 and Windows Server AMIs) to establish an encrypted channel through the SSM endpoint — access is gated by IAM permissions on the ssm:StartSession action rather than network accessibility. All session activity (commands and output) can be logged to S3 and CloudWatch Logs. Session Manager removes the attack surface of an internet-facing bastion host and eliminates the operational overhead of key rotation and bastion patching."
  - question: "How do I enforce least privilege for privileged access in AWS?"
    answer: "Enforce least privilege for privileged access by: using permission boundaries to cap the maximum permissions any role can have, even if the IAM policy attached to it is broader; applying SCPs at the Organizational Unit level to prevent accounts from creating roles with AdministratorAccess without explicit approval; using IAM Access Analyzer to identify roles with unused privilege and generate least-privilege policy suggestions based on actual CloudTrail activity; requiring MFA as a condition on all sensitive IAM actions (iam:*, sts:AssumeRole to production accounts); and auditing role usage weekly to identify roles that have not been assumed and should be removed or scoped down."
draft: false
---
